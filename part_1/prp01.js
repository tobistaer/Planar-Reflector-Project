...
// The reflection is done by reflecting the *model matrix* around the plane, which is simple and
// keeps the view/projection identical for both teapot and its mirror image.
...
function perspectiveFovY(fovy, aspect, near, far){ const f=1/Math.tan(fovy/2), nf=1/(near-far); const m=new Float32Array(16); m[0]=f/aspect; m[5]=f; m[10]=(far+near)*nf; m[11]=-1; m[14]=2*far*near*nf; return m; }
function lookAt(e,c,u){ const[ex,ey,ez]=e,[cx,cy,cz]=c,[ux,uy,uz]=u; let zx=ex-cx, zy=ey-cy, zz=ez-cz; { const l=1/Math.hypot(zx,zy,zz); zx*=l; zy*=l; zz*=l; } let xx=uy*zz-uz*zy, xy=uz*zx-ux*zz, xz=ux*zy-uy*zx; { const l=1/Math.hypot(xx,xy,xz); xx*=l; xy*=l; xz*=l; } const yx=zy*xz-zz*xy, yy=zz*xx-zx*xz, yz=zx*xy-zy*xx; const m=I4(); m[0]=xx; m[4]=xy; m[8]=xz; m[1]=yx; m[5]=yy; m[9]=yz; m[2]=zx; m[6]=zy; m[10]=zz; m[12]=-(xx*ex+xy*ey+xz*ez); m[13]=-(yx*ex+yy*ey+yz*ez); m[14]=-(zx*ex+zy*ey+zz*ez); return m; }
function mat4Transpose(m){ const t=new Float32Array(16); for(let r=0;r<4;r++) for(let c=0;c<4;c++) t[c*4+r]=m[r*4+c]; return t; }
function mat4Inverse(a){ const m=new Float32Array(16); const b=new Float32Array(16); b.set(a);
...
function makeBuffer(data, usage){ const buffer = device.createBuffer({ size:((data.byteLength+3)&~3), usage }); device.queue.writeBuffer(buffer,0,data); return buffer; }
...
// Base texture repeats so the ground can be tiled (used in later parts).
...
const shadowSampler = device.createSampler({ addressModeU:'clamp-to-edge', addressModeV:'clamp-to-edge', magFilter:'nearest', minFilter:'nearest' });
...
const shaderCode = await (await fetch('./prp01.wgsl?v=shadowfix2')).text();
const shaderModule = device.createShaderModule({ code: shaderCode });

// We keep a single "lit" bind group layout for ground + teapot shaders, even though Part 1
// only draws the teapot. This keeps the shader interface identical across parts.
const litLayout = device.createBindGroupLayout({ entries:[
...
const groundPipeline = await device.createRenderPipelineAsync({
  ...
  vertex:{ module:shaderModule, entryPoint:'vsGround', buffers:[
  ...
  fragment:{ module:shaderModule, entryPoint:'fsGround', targets:[{ format }] },
...
const teapotPipeline = await device.createRenderPipelineAsync({
  layout: device.createPipelineLayout({ bindGroupLayouts:[litLayout] }),
  vertex:{ module:shaderModule, entryPoint:'vsTeapot', buffers:[
  ...
  fragment:{ module:shaderModule, entryPoint:'fsTeapot', targets:[{ format }] },
...
const shadowPipeline = await device.createRenderPipelineAsync({
  ...
  vertex:{ module:shaderModule, entryPoint:'vsShadow', buffers:[
    { arrayStride:16, attributes:[{ shaderLocation:0, offset:0, format:'float32x4' }] },
  ]},
  fragment:{ module:shaderModule, entryPoint:'fsShadow', targets:[{ format:'rgba32float' }] },
...
const groundBindGroup = device.createBindGroup({ layout:litLayout, entries:[
...
const teapotBindGroup = device.createBindGroup({ layout:litLayout, entries:[
...
const reflectedTeapotBindGroup = device.createBindGroup({ layout:litLayout, entries:[
...
const shadowTeapotBindGroup = device.createBindGroup({ layout:shadowLayout, entries:[
...
let depthTex = device.createTexture({ size:{width:canvas.width,height:canvas.height}, format:'depth24plus', usage:GPUTextureUsage.RENDER_ATTACHMENT });
function resizeIfNeeded(){
  ...
  if(canvas.width !== displayWidth || canvas.height !== displayHeight){
    ...
    depthTex.destroy();
    depthTex = device.createTexture({ size:{width:displayWidth,height:displayHeight}, format:'depth24plus', usage:GPUTextureUsage.RENDER_ATTACHMENT });
    updateViewProj();
  }
}
