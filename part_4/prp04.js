...
const shaderCode = await (await fetch('./prp04.wgsl?v=shadowfix3')).text();
...
let viewProj = mat4Mul(proj, view);

function updateViewProj(){
  const aspect = canvas.width / canvas.height;
  proj = perspectiveFovY(65 * Math.PI/180, aspect, 0.1, 100.0);
...
function frame(ts){
  ...
  // Note: Part 4 uses a dedicated fullscreen pipeline to show the depth map, instead of trying to
  // shoehorn it into the scene shaders (which would require a proper screen-space UV).
  shadowParams[2] = 0;
  ...
  device.queue.writeBuffer(reflectedTeapotUBO, 272, new Float32Array([...eye, 1]));
  ...
  pass1.setVertexBuffer(1, groundUVBuf);
  pass1.setVertexBuffer(2, groundNormalBuf);
  ...
  pass1.setVertexBuffer(1, teapotNrmBuf);
  ...
  pass2.setVertexBuffer(1, groundUVBuf);
