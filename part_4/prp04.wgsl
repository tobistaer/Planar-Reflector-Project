...
// When the UI says "Show depth map" we want to display the raw shadow texture 1:1 on screen.
// The regular scene shaders sample the shadow map in light space; they don't have a natural UV
// that matches the shadow texture, so we render a fullscreen quad instead.
